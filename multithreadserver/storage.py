    ...
    def __init__(self,host,port):
        ...
        try: 
            #while not self.stop:
                ...
                time.sleep(1)
    def senddata(self,datatosend,charge):
...
while True:
    ...
    if timer_elapsed == 1: # This is executed at 1kHz
        ...
        if CL != 1: # Buck-OL Open loop so just limit the current but otherwise pass through the reference directly as a duty cycle
            ...
            if iL > 0.35: # Current limiting function
        ...
        CEnergy= 0.5*0.5*vb*vb #Energy in Capacitor


        
        CMode= 1# 1 if charging 0 if discharging
        DMode = 0 # 1 if want capacitor to discharge
        
        # Will later change modlue to discharge by x amount
        # can also make an automatic module that capacitor will always charge when its voltage is low unless it has been told to discharge
        ...
        if DMode == 1 & CMode ==0:
            ...
            duty=29500 # THIS IS THE VALUE THAT WILL PROBABLY CHANGE what to model duty cyle operating range for Ib = 0 not storing and discharging.
        ...
        if count > 100:
            ...
            print("Capacitor Energy = {:.3f}".format(CEnergy))
